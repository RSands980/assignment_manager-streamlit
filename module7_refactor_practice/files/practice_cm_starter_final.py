...
Phase 5 version:
- CodeStore handles JSON loading and saving.
- CodeService handles the code list and business rules.
- CodeDashboard handles the Streamlit UI and saving after successful changes.
- main() only connects the objects and starts the app.
"""
...
class CodeStore:
    """Handles loading and saving code records from the JSON file."""
    ...
    def save(self, codes: list[dict[str, Any]]) -> None:
        with open(self.json_path, "w", encoding="utf-8") as file:
            json.dump(codes, file, indent=2)


class CodeService:
    """Handles code records and business rules."""

    def __init__(self, initial_codes: list[dict[str, Any]]) -> None:
        self._codes = initial_codes
    ...
    def use_code(self, code: str, user_id: str) -> dict[str, Any]:
        ...
        if (
            code_data["max_uses"]
            and code_data["current_uses"] >= code_data["max_uses"]
        ):
...
class CodeDashboard:
    """Handles the Streamlit UI and coordinates saving."""

    def __init__(self, service: CodeService, store: CodeStore) -> None:
    ...
    def save_changes(self) -> None:
        self.store.save(self.service.all_codes())
    ...
    def show_generate_tab(self, course_id: str, teacher_id: str) -> None:
        ...
        if recent_codes:
            st.dataframe(
                self.format_codes_for_display(recent_codes[:5]),
    ...
    def show_manage_tab(self, course_id: str) -> None:
        ...
        if codes:
            st.dataframe(
                self.format_codes_for_display(codes),
                use_container_width=True,
                hide_index=True,
            )
        else:
        ...
        active_codes = [
            item["code"] for item in self.service.get_codes(course_id, active_only=True)
        ]
...
def main() -> None:
    ...
    service = CodeService(store.load())
