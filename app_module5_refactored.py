...
def save_data(json_path, assignments):
...
def render_add_new_assignment(assignments, json_path):
    ...
    if st.button("Save Assignment", key="save_assignment_btn", use_container_width=True):
        with st.spinner("Saving Assignment..."):
            ...
            assignments = add_new_assignment(assignments,st.session_state["draft"]["title"], st.session_state["draft"]["description"], st.session_state["draft"]["points"], st.session_state["draft"]["type"])

        save_data(json_path, assignments)
